...
for case_num in set(table['case_number']):
    try:
        ...
        patient = deepdrr.Volume.from_nifti(f"sample_datasets/{case_num}_BONE_H-N-UXT_3X3.nii.gz", use_thresholding=True)
        patient.facedown()
        ...
        filtered_table = table[table.apply(lambda row: 'BONE_H-N-UXT_3X3' in row.values, axis=1) & (table['case_number'] == case_num)]
        cv2.imwrite(f"whole_body/H/{case_num}.png", combined_image)
